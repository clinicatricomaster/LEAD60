const app = {
    ...
    FUNIL_STATUS: ['Lead de Entrada', 'Primeiro Contato', 'Qualificação', 'Agendamento', 'Consulta Realizada', 'Protocolo Venda', 'Não Compareceu', 'Perdido'],
    FUNIL_STATUS_DISPLAY: ['Lead de Entrada', 'Primeiro Contato', 'Qualificação', 'Agendamento', 'Consulta Realizada', 'Não Compareceu'],
    ...
    setupLoginListeners() {
        ...
        if (loginForm && !loginForm.dataset.listenerAttached) {
            loginForm.addEventListener('submit', (e) => {
                e.preventDefault();
                const action = e.submitter ? e.submitter.dataset.action : 'login';
                if (action === 'login') {
                    this.handleLogin();
                }
            });
    ...
    setupEventListeners() {
        ...
        const filters = ['dash-filtro-data-inicio', 'dash-filtro-data-fim', 'dash-filtro-unidade', 'dash-filtro-status', 'dash-filtro-origem', 'leads-filtro-origem', 'leads-filtro-status', 'leads-filtro-unidade', 'leads-filtro-medico', 'medicos-filtro-data-inicio', 'medicos-filtro-data-fim'];
        filters.forEach(id => {
    ...
    converterDataKommo(dataString) {
        if (!dataString || typeof dataString !== 'string') return new Date().toISOString();
        try {
            const parteData = dataString.split(' ')[0];
            const [dia, mes, ano] = parteData.split('.').map(Number);
            return new Date(ano, mes - 1, dia).toISOString();
        } catch (e) {
            console.error(`Erro ao converter data: "${dataString}". Usando data atual.`, e);
            return new Date().toISOString();
        }
    },

    converterTelefoneKommo(telefoneString) {
        if (!telefoneString || typeof telefoneString !== 'string') return '';
        let digitos = telefoneString.replace(/\D/g, '');
        ...
        reader.onload = async (e) => {
            try {
                ...
                importedData.forEach(linha => {
                    const newLead = {
                        dataCriacao: this.converterDataKommo(linha.dataCriacao),
                        telefone: this.converterTelefoneKommo(String(linha.telefone || '')),
                        protocoloVendido: linha.protocoloVendido || 'Não',
                        valorProtocolo: linha.valorProtocolo || 0,
                        nome: this.toTitleCase(linha.nome) || 'Sem nome',
                        origem: linha.origem || 'Outros',
                        status: linha.status || 'Lead de Entrada',
                        medico: linha.medico || '',
                ...
                this.data.leads.sort((a, b) => new Date(b.dataCriacao) - new Date(a.dataCriacao));
                this.renderGerenciarLeads();
                alert(`${importedData.length} leads foram importados e salvos com sucesso!`);
            } catch (error) {
    ...
    exportToExcel() {
        if (this.data.leads.length === 0) { alert("Não há leads para exportar."); return; }
        const dataToExport = this.data.leads.map(({ id, ...lead }) => lead);
        const worksheet = XLSX.utils.json_to_sheet(dataToExport);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Leads");
        const fileName = `TricoMaster_Leads_${new Date().toISOString().split('T')[0]}.xlsx`;
        XLSX.writeFile(workbook, fileName);
    },
    
    exportarMetricasParaExcel() {
        const filters = this.getFilters('dash');
        const filteredLeads = this.filterLeads(filters);
        const kpis = this.calculateKPIs(filteredLeads, filters);
        const kpiData = [["Métrica", "Valor"], ["Total de Leads", kpis.totalLeads], ["Consultas Agendadas", kpis.consultasAgendadas], ["Consultas Realizadas", kpis.consultasRealizadas], ["Protocolos Vendidos", kpis.protocolosVendidos], ["CPL", this.formatCurrency(kpis.cpl)], ["CPA", this.formatCurrency(kpis.cpa)], ["Faturamento", this.formatCurrency(kpis.faturamento)]];
        const funilData = [["Estágio do Funil", "Quantidade"]];
        const leadsParaFunil = filteredLeads.map(lead => lead.status === 'Protocolo Venda' ? { ...lead, status: 'Consulta Realizada' } : lead);
        const statusParaFunil = this.FUNIL_STATUS.filter(s => s !== 'Protocolo Venda' && s !== 'Perdido');
        statusParaFunil.forEach(status => {
            const count = leadsParaFunil.filter(lead => lead.status === status).length;
            funilData.push([status, count]);
        });
        const origemData = [["Origem", "Quantidade"]];
        this.ORIGENS.forEach(origem => {
            const count = filteredLeads.filter(lead => lead.origem === origem).length;
        ...
        XLSX.utils.book_append_sheet(workbook, wsKPIs, "KPIs Principais");
        XLSX.utils.book_append_sheet(workbook, wsFunil, "Funil de Vendas");
        XLSX.utils.book_append_sheet(workbook, wsOrigem, "Leads por Origem");
        const dataFim = filters.dataFim ? filters.dataFim.toISOString().split('T')[0] : 'geral';
        XLSX.writeFile(workbook, `TricoMaster_Metricas_${dataFim}.xlsx`);
    },

    exportarComparativosParaExcel() {
        const workbook = XLSX.utils.book_new();
        const mes1ISO = document.getElementById('comp-mes1').value;
        const mes2ISO = document.getElementById('comp-mes2').value;
        if (mes1ISO && mes2ISO) {
            const metricaMes1 = this.getMetricsForLeads(this.data.leads.filter(l => l.dataCriacao.startsWith(mes1ISO)));
            const metricaMes2 = this.getMetricsForLeads(this.data.leads.filter(l => l.dataCriacao.startsWith(mes2ISO)));
            const mesAMesData = [["Métrica", mes1ISO, mes2ISO]];
            mesAMesData.push(["Total de Leads", metricaMes1.totalLeads, metricaMes2.totalLeads]);
            mesAMesData.push(["Consultas Realizadas", metricaMes1.consultasRealizadas, metricaMes2.consultasRealizadas]);
            mesAMesData.push(["Protocolos Vendidos", metricaMes1.protocolosVendidos, metricaMes2.protocolosVendidos]);
            mesAMesData.push(["Faturamento", this.formatCurrency(metricaMes1.faturamento), this.formatCurrency(metricaMes2.faturamento)]);
            mesAMesData.push([]);
            mesAMesData.push(["Funil de Vendas"]);
            this.FUNIL_STATUS_DISPLAY.forEach(status => {
                mesAMesData.push([status, metricaMes1.funil[status], metricaMes2.funil[status]]);
            });
            const wsMesAMes = XLSX.utils.aoa_to_sheet(mesAMesData);
            XLSX.utils.book_append_sheet(workbook, wsMesAMes, "Mês a Mês");
        }
        const mesSemanalISO = document.getElementById('comp-semanal-mes').value;
        if (mesSemanalISO) {
            const [ano, mes] = mesSemanalISO.split('-').map(Number);
            ...
            const semanalData = [["Métrica", "Semana 1", "Semana 2", "Semana 3", "Semana 4"]];
            semanalData.push(["Total Leads", semanas[0].totalLeads, semanas[1].totalLeads, semanas[2].totalLeads, semanas[3].totalLeads]);
            semanalData.push(["Faturamento", this.formatCurrency(semanas[0].faturamento), this.formatCurrency(semanas[1].faturamento), this.formatCurrency(semanas[2].faturamento), this.formatCurrency(semanas[3].faturamento)]);
            semanalData.push([]);
            semanalData.push(["Funil de Vendas"]);
            this.FUNIL_STATUS_DISPLAY.forEach(status => {
                semanalData.push([status, semanas[0].funil[status], semanas[1].funil[status], semanas[2].funil[status], semanas[3].funil[status]]);
            });
            const wsSemanal = XLSX.utils.aoa_to_sheet(semanalData);
            XLSX.utils.book_append_sheet(workbook, wsSemanal, "Semanal " + mesSemanalISO);
        }
        XLSX.writeFile(workbook, `TricoMaster_Comparativos_${new Date().toISOString().split('T')[0]}.xlsx`);
    },
    ...
    getMetricsForLeads(leadsArray) {
        ...
        const funilCalcs = this.FUNIL_STATUS.reduce((acc, status) => {
            acc[status] = leadsArray.filter(l => l.status === status).length;
            return acc;
        }, {});
        funilCalcs['Consulta Realizada'] += funilCalcs['Protocolo Venda'];
        return { totalLeads: leadsArray.length, consultasAgendadas, consultasRealizadas, protocolosVendidos, faturamento, funil: funilCalcs, origens: this.ORIGENS.reduce((acc, origem) => { acc[origem] = leadsArray.filter(l => l.origem === origem).length; return acc; }, {}), };
    },
    ...
    filterLeads(filters) {
        ...
        return this.data.leads.filter(lead => {
            if (termoBusca) {
                ...
                const telefoneLead = lead.telefone ? lead.telefone.replace(/\D/g, '') : '';
    ...
    populateFilters(prefix) {
        ...
        if (document.getElementById(`${prefix}-filtro-origem`)) this.populateSelect(`${prefix}-filtro-origem`, this.ORIGENS, true, "Todas Origens");
    ...
    populateSelect(elementId, options, withAllOption, allOptionText) {
        ...
        if (withAllOption) {
            ...
            option.value = '';
            option.innerText = allOptionText;
    ...
    renderChart(elementId, title, categories, data, field, barColorClass) {
        const container = document.getElementById(elementId);
        if(!container) return;
        const total = data.filter(item => categories.includes(item[field])).length;
        let chartHTML = `<h3 class="text-xl font-semibold text-tm-texto-principal mb-4">${title}</h3><div class="space-y-3">`;
        categories.forEach(category => {
            const count = data.filter(item => item[field] === category).length;
            const percentage = total > 0 ? ((count / total) * 100).toFixed(1) : 0;
            chartHTML += `<div class="w-full"><div class="flex justify-between mb-1 text-sm font-medium text-tm-texto-secundario"><span>${category}</span><span>${count} (${percentage}%)</span></div><div class="bg-gray-200 rounded-full h-2.5"><div class="${barColorClass} h-2.5 rounded-full" style="width: ${percentage}%"></div></div></div>`;
        });
        chartHTML += '</div>';
    ...
    openLeadModal(leadId = null) {
        ...
        this.populateSelect('lead-origem', this.ORIGENS, false);
        this.populateSelect('lead-status', this.FUNIL_STATUS, false);
        this.populateSelect('lead-unidade', this.UNIDADES, false);
        this.populateSelect('lead-medico', this.data.medicos.map(m => m.nome), false, 'Nenhum');
        if (leadId) {
            ...
            document.getElementById('lead-data').value = new Date(lead.dataCriacao).toISOString().split('T')[0];
            document.getElementById('lead-telefone').value = lead.telefone;
            document.getElementById('lead-origem').value = lead.origem;
            document.getElementById('lead-status').value = lead.status;
            document.getElementById('lead-medico').value = lead.medico;
            document.getElementById('lead-unidade').value = lead.unidade;
            document.getElementById('lead-protocolo').value = lead.protocoloVendido || 'Não';
            document.getElementById('lead-valor-protocolo').value = lead.valorProtocolo;
        } else {
            this.state.editingLeadId = null;
            document.getElementById('modal-title').innerText = 'Adicionar Novo Lead';
            document.getElementById('lead-protocolo').value = 'Não';
            document.getElementById('lead-data').value = new Date().toISOString().split('T')[0];
        }
        document.getElementById('modal-novo-lead').classList.remove('hidden');
        document.getElementById('modal-novo-lead').classList.add('flex');
    },

    closeLeadModal() {
        document.getElementById('modal-novo-lead').classList.add('hidden');
        document.getElementById('modal-novo-lead').classList.remove('flex');
    },
    ...
    renderMetricas() {
        ...
        kpisContainer.className = "grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6 mb-6";
        ...
        this.renderChart('chart-origem', 'Leads por Origem', this.ORIGENS, filteredLeads, 'origem', 'bg-tm-dourado');
    },
    ...
    renderComparativo() {
        ...
        const mesAnterior = String(hoje.getMonth() + 1).padStart(2, '0');
        document.getElementById('comp-mes1').value = `${anoAnterior}-${mesAnterior}`;
        document.getElementById('comp-mes2').value = `${anoAtual}-${mesAtual}`;
        document.getElementById('comp-semanal-mes').value = `${anoAtual}-${mesAtual}`;
        const fimArco = new Date();
        const inicioArco = new Date();
        inicioArco.setMonth(fimArco.getMonth() - 3);
        inicioArco.setDate(1);
        document.getElementById('comp-arco-inicio').value = inicioArco.toISOString().split('T')[0];
        document.getElementById('comp-arco-fim').value = fimArco.toISOString().split('T')[0];
        this.renderCompMesAMes();
        this.renderCompArco();
    ...
    renderCompMesAMes() {
        ...
        this.ORIGENS.forEach(origem => { tableHTML += criarLinha({ nome: origem, m1: metricaMes1.origens[origem], m2: metricaMes2.origens[origem], format: v => v }); });
        tableHTML += '</tbody></table>';
        container.innerHTML = tableHTML;
    },
    
    renderCompArco() {
        const container = document.getElementById('comparativo-arco-content');
        const inicioStr = document.getElementById('comp-arco-inicio').value;
        const fimStr = document.getElementById('comp-arco-fim').value;
        if (!container || !inicioStr || !fimStr) return;
        const dataInicio = new Date(inicioStr + 'T00:00:00');
        const dataFim = new Date(fimStr + 'T23:59:59');
        const leadsNoArco = this.data.leads.filter(lead => { const dataLead = new Date(lead.dataCriacao); return dataLead >= dataInicio && dataLead <= dataFim; });
        if (leadsNoArco.length === 0) { container.innerHTML = '<p class="text-tm-texto-secundario">Nenhum dado encontrado para este período.</p>'; return; }
        const totaisArco = this.getMetricsForLeads(leadsNoArco);
        let totaisHTML = ` <h4 class="font-semibold text-lg mb-2">Totais do Período</h4> <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6 text-center"> <div class="p-4 bg-gray-50 rounded-lg"><div class="text-sm text-tm-texto-secundario">Total Leads</div><div class="text-2xl font-bold text-tm-verde">${totaisArco.totalLeads}</div></div> <div class="p-4 bg-gray-50 rounded-lg"><div class="text-sm text-tm-texto-secundario">Consultas Realizadas</div><div class="text-2xl font-bold text-tm-verde">${totaisArco.consultasRealizadas}</div></div> <div class="p-4 bg-gray-50 rounded-lg"><div class="text-sm text-tm-texto-secundario">Faturamento</div><div class="text-2xl font-bold text-tm-verde">${this.formatCurrency(totaisArco.faturamento)}</div></div> <div class="p-4 bg-gray-50 rounded-lg"><div class="text-sm text-tm-texto-secundario">Google Ads</div><div class="text-2xl font-bold text-tm-verde">${totaisArco.origens['Google Ads'] || 0}</div></div> <div class="p-4 bg-gray-50 rounded-lg"><div class="text-sm text-tm-texto-secundario">Instagram</div><div class="text-2xl font-bold text-tm-verde">${totaisArco.origens['Instagram'] || 0}</div></div> </div> <hr class="my-6">`;
        const dadosMensais = {};
        let dataCorrente = new Date(dataInicio);
        while (dataCorrente <= dataFim) { const mesISO = dataCorrente.getFullYear() + '-' + String(dataCorrente.getMonth() + 1).padStart(2, '0'); dadosMensais[mesISO] = { totalLeads: 0, faturamento: 0, googleLeads: 0, instagramLeads: 0 }; dataCorrente.setMonth(dataCorrente.getMonth() + 1); }
        const medicosMap = new Map(this.data.medicos.map(m => [m.nome, m.valorConsulta]));
        leadsNoArco.forEach(lead => { const dataLead = new Date(lead.dataCriacao); const mesISO = dataLead.getFullYear() + '-' + String(dataLead.getMonth() + 1).padStart(2, '0'); if (dadosMensais[mesISO]) { let faturamentoLead = 0; if (lead.protocoloVendido === 'Sim') { faturamentoLead += (lead.valorProtocolo || 0); } if (['Consulta Realizada', 'Protocolo Venda'].includes(lead.status) && lead.medico) { faturamentoLead += (medicosMap.get(lead.medico) || 0); } dadosMensais[mesISO].faturamento += faturamentoLead; dadosMensais[mesISO].totalLeads += 1; if (lead.origem === 'Google Ads') dadosMensais[mesISO].googleLeads += 1; if (lead.origem === 'Instagram') dadosMensais[mesISO].instagramLeads += 1; } });
        const renderChartEvolucao = (titulo, data, chave, cor, formatFn) => { const valores = Object.values(data).map(item => item[chave]); const maxValor = Math.max(...valores, 1); let chartHTML = `<div class="mb-6"><h4 class="font-semibold text-lg mb-2">${titulo}</h4><div class="flex items-end h-40 space-x-2 border-l border-b border-gray-300 p-2">`; for (const mes in data) { const valor = data[mes][chave]; const altura = (valor / maxValor) * 100; chartHTML += ` <div class="flex-1 flex flex-col items-center justify-end" title="${mes}: ${formatFn(valor)}"> <div class="text-xs font-bold -mb-1">${formatFn(valor)}</div> <div class="${cor} w-full rounded-t-sm" style="height: ${altura}%"></div> <div class="text-xs text-tm-texto-secundario mt-1">${mes.substring(5)}/${mes.substring(2,4)}</div> </div>`; } chartHTML += '</div></div>'; return chartHTML; };
        const renderLeadsGroupedChart = () => { let chartHTML = `<div class="mb-6"><h4 class="font-semibold text-lg mb-2">Evolução de Leads (Total vs. Origens Principais)</h4>`; chartHTML += `<div class="flex items-center space-x-4 mb-2 text-sm"> <div class="flex items-center"><span class="w-3 h-3 rounded-full bg-tm-verde mr-2"></span>Total</div> <div class="flex items-center"><span class="w-3 h-3 rounded-full bg-blue-500 mr-2"></span>Google Ads</div> <div class="flex items-center"><span class="w-3 h-3 rounded-full bg-pink-500 mr-2"></span>Instagram</div> </div>`; chartHTML += `<div class="flex items-stretch h-64 space-x-2 border-l border-b border-gray-300 p-2">`; const allValues = Object.values(dadosMensais).flatMap(d => [d.totalLeads, d.googleLeads, d.instagramLeads]); const maxValor = Math.max(...allValues, 1); for (const mes in dadosMensais) { const data = dadosMensais[mes]; const hTotal = (data.totalLeads / maxValor) * 100; const hGoogle = (data.googleLeads / maxValor) * 100; const hInstagram = (data.instagramLeads / maxValor) * 100; chartHTML += `<div class="flex-1 flex flex-col items-center"> <div class="w-full flex-grow flex items-end justify-center space-x-1"> <div title="Total: ${data.totalLeads}" class="bg-tm-verde w-1/3" style="height: ${hTotal}%;"></div> <div title="Google Ads: ${data.googleLeads}" class="bg-blue-500 w-1/3" style="height: ${hGoogle}%;"></div> <div title="Instagram: ${data.instagramLeads}" class="bg-pink-500 w-1/3" style="height: ${hInstagram}%;"></div> </div> <div class="text-xs text-tm-texto-secundario mt-1">${mes.substring(5)}/${mes.substring(2,4)}</div> </div>`; } chartHTML += '</div></div>'; return chartHTML; };
        let graficosHTML = renderChartEvolucao('Evolução do Faturamento', dadosMensais, 'faturamento', 'bg-tm-verde', v => this.formatCurrency(v));
        graficosHTML += renderLeadsGroupedChart();
        container.innerHTML = totaisHTML + graficosHTML;
    },
    ...
    renderGerenciarLeads() {
        ...
        paginatedLeads.forEach(lead => {
            ...
            tr.innerHTML = `
                <td class="p-3">${new Date(lead.dataCriacao).toLocaleDateString()}</td><td class="p-3">${lead.nome}</td><td class="p-3">${lead.telefone || ''}</td><td class="p-3">${lead.origem}</td>
                <td class="p-3"><select data-quick-edit="true" data-field="status" data-id="${lead.id}" class="p-1 border rounded-md w-full bg-white">${this.FUNIL_STATUS.map(s => `<option value="${s}" ${s === lead.status ? 'selected' : ''}>${s}</option>`).join('')}</select></td>
                <td class="p-3"><select data-quick-edit="true" data-field="unidade" data-id="${lead.id}" class="p-1 border rounded-md w-full bg-white">${this.UNIDADES.map(u => `<option value="${u}" ${u === lead.unidade ? 'selected' : ''}>${u}</option>`).join('')}</select></td>
                <td class="p-3"><select data-quick-edit="true" data-field="medico" data-id="${lead.id}" class="p-1 border rounded-md w-full bg-white"><option value="">Nenhum</option>${this.data.medicos.map(m => `<option value="${m.nome}" ${m.nome === lead.medico ? 'selected' : ''}>${m.nome}</option>`).join('')}</select></td>
                <td class="p-3"><select data-quick-edit="true" data-field="protocoloVendido" data-id="${lead.id}" class="p-1 border rounded-md w-full bg-white"><option value="Não" ${lead.protocoloVendido === 'Não' ? 'selected' : ''}>Não</option><option value="Sim" ${lead.protocoloVendido === 'Sim' ? 'selected' : ''}>Sim</option></select></td>
                <td class="p-3 text-center"><button data-action="open-lead-modal" data-id="${lead.id}" class="text-tm-azul hover:text-tm-azul-hover"><i class="fas fa-edit"></i></button><button data-action="delete-lead" data-id="${lead.id}" class="text-tm-bordo hover:opacity-80 ml-3"><i class="fas fa-trash"></i></button></td>`;
            body.appendChild(tr);
        });

const app = {
    ...
    PROTOCOLOS: ['Sim', 'Não'],
    // NOVA CONSTANTE PARA EXCLUSÃO
    ORIGENS_EXCLUIDAS: ['acompanhar', 'pac tricomaster'],
    ...
    async loadDataFromFirebase() {
        try {
            ...
            // Popula a lista de origens dinamicamente
    ...
    async importFromExcel(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (e) => { try { const data = new Uint8Array(e.target.result); const workbook = XLSX.read(data, { type: 'array' }); const sheetName = workbook.SheetNames[0]; const worksheet = workbook.Sheets[sheetName]; const importedData = XLSX.utils.sheet_to_json(worksheet); if (importedData.length === 0) { alert("A planilha selecionada está vazia."); return; } if (!importedData[0] || importedData[0]["Contato principal"] === undefined || importedData[0]["Etapa do lead"] === undefined) { alert("Erro: A planilha não parece ser o modelo correto. Colunas como 'Contato principal' ou 'Etapa do lead' não foram encontradas."); event.target.value = ''; return; } const existingPhonesSet = new Set(this.data.leads.map(lead => this.getRawDigits(lead.telefone))); const batch = this.db.batch(); const newLeadsForState = []; let duplicados = 0; let adicionados = 0; importedData.forEach(linha => { const newPhoneFormatted = this.converterTelefoneKommo(String(linha["Telefone comercial (contato)"] || '')); const newPhoneRaw = this.getRawDigits(newPhoneFormatted); if (newPhoneRaw && existingPhonesSet.has(newPhoneRaw)) { duplicados++; } else { adicionados++; const newLead = { nome: this.toTitleCase(linha["Contato principal"]) || 'Sem nome', dataCriacao: this.converterDataKommo(linha["Data Criada"]), telefone: newPhoneFormatted, status: linha["Etapa do lead"] || 'Lead de Entrada', origem: linha["Lead tags"] || 'Outros', medico: this.toTitleCase(linha["Nota 1"]) || '', unidade: this.toTitleCase(linha["Nota 2"]) || '', protocoloVendido: linha["Nota 3"] || 'Não', valorProtocolo: parseFloat(linha["Nota 4"]) || 0, }; const docRef = this.db.collection('leads').doc(); batch.set(docRef, newLead); newLeadsForState.push({ id: docRef.id, ...newLead }); if (newPhoneRaw) { existingPhonesSet.add(newPhoneRaw); } } }); if (adicionados > 0) { await batch.commit(); this.data.leads.push(...newLeadsForState); this.data.leads.sort((a, b) => new Date(b.dataCriacao) - new Date(a.dataCriacao)); this.clearLeadsFilters(); this.renderGerenciarLeads(); } alert(`Importação concluída!\n\nLeads Adicionados: ${adicionados}\nLeads Duplicados (ignorados): ${duplicados}`); } catch (error) { console.error("Erro ao importar a planilha:", error); alert("Ocorreu um erro ao processar o arquivo."); } finally { event.target.value = ''; } }; reader.readAsArrayBuffer(file); },
    ...
    clearLeadsFilters() {
        document.getElementById('leads-filtro-busca').value = '';
        document.getElementById('leads-filtro-data-inicio').value = '';
        document.getElementById('leads-filtro-data-fim').value = '';
        document.getElementById('leads-filtro-origem').value = '';
        document.getElementById('leads-filtro-status').value = '';
        document.getElementById('leads-filtro-unidade').value = '';
        document.getElementById('leads-filtro-medico').value = '';
        document.getElementById('leads-filtro-protocolo').value = '';
    ...
    renderMetricas() {
        ...
        const statusParaFunil = this.FUNIL_STATUS_DISPLAY;
        
        const origensExcluidas = this.ORIGENS_EXCLUIDAS.map(o => o.toLowerCase());
        const leadsParaGraficoOrigem = filteredLeads.filter(lead => !origensExcluidas.includes((lead.origem || '').toLowerCase()));
        const origensParaGrafico = this.data.origens.filter(origem => !origensExcluidas.includes((origem || '').toLowerCase()));

        this.renderChart('chart-funil', 'Funil de Vendas', statusParaFunil, leadsParaFunil, 'status', 'bg-tm-verde');
        this.renderChart('chart-origem', 'Leads por Origem', origensParaGrafico, leadsParaGraficoOrigem, 'origem', 'bg-tm-dourado');
    },
